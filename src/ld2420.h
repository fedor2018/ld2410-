...
#define LD2420_MAX_FRAME_LENGTH 40
...
class ld2420	{
	...
	public:
		ld2420();														//Constructor function
		~ld2420();														//Destructor function
		bool begin(Stream &, bool waitForRadar = true);					//Start the ld2420
	...
	private:
		...
		uint32_t radar_uart_timeout = 100;								//How long to give up on receiving some useful data from the LD2420
		...
		uint8_t radar_data_frame_[LD2420_MAX_FRAME_LENGTH];				//Store the incoming data from the radar, to check it's in a valid format
