/*
 *	An Arduino library for the Hi-Link LD2420 24Ghz FMCW radar sensor.
 *
...
ld2420::ld2420()	//Constructor function
{
}

ld2420::~ld2420()	//Destructor function
{
}
...
bool ld2420::stationaryTargetDetected()
{
...
uint16_t ld2420::stationaryTargetDistance()
{
...
uint8_t ld2420::stationaryTargetEnergy()
{
...
bool ld2420::movingTargetDetected()
{
...
uint16_t ld2420::movingTargetDistance()
{
...
uint8_t ld2420::movingTargetEnergy()
{
...
bool ld2420::read_frame_()
{
	if(radar_uart_ -> available())
	{
		...
		{
			if(radar_data_frame_position_ < LD2420_MAX_FRAME_LENGTH)
			{
				#ifdef LD2420_DEBUG_DATA
			...
			{
				#if defined(LD2420_DEBUG_DATA) || defined(LD2420_DEBUG_COMMANDS)
				if(debug_uart_ != nullptr)
				{
					debug_uart_->print(F("\nLD2420 frame overran"));
				}
				#endif
				frame_started_ = false;
				radar_data_frame_position_ = 0;
...
bool ld2420::enter_configuration_mode_()
{
...
bool ld2420::leave_configuration_mode_()
{
	send_command_preamble_();
	//Request firmware
...
bool ld2420::requestStartEngineeringMode()
{
	send_command_preamble_();
	//Request firmware
	radar_uart_->write((byte) 0x02);	//Command is two bytes long
...
bool ld2420::requestEndEngineeringMode()
{
	send_command_preamble_();
	//Request firmware
	radar_uart_->write((byte) 0x02);	//Command is two bytes long
...
bool ld2420::requestCurrentConfiguration()
{
...
bool ld2420::requestFirmwareVersion()
{
	if(enter_configuration_mode_())
	{
		...
		radar_uart_->write((byte) 0x02);	//Command is two bytes long
		radar_uart_->write((byte) 0x00);
		radar_uart_->write((byte) 0x00);	//Request firmware version
		radar_uart_->write((byte) 0x00);
		send_command_postamble_();
		radar_uart_last_command_ = millis();
		while(millis() - radar_uart_last_command_ < radar_uart_command_timeout_)
		{
			read_frame_();
			if(latest_ack_ == 0x00 && latest_command_success_)
			{
				delay(50);
				leave_configuration_mode_();
				return true;
...
bool ld2420::requestFactoryReset()
{
	if(enter_configuration_mode_())
	{
		delay(50);
...
bool ld2420::setMaxValues(uint16_t moving, uint16_t stationary, uint16_t inactivityTimer)
{
	if(enter_configuration_mode_())
	{
		delay(50);
...
bool ld2420::setGateSensitivityThreshold(uint8_t gate, uint8_t moving, uint8_t stationary)
{
	if(enter_configuration_mode_())
	{
		delay(50);
